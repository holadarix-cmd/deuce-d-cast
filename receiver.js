(() => {
  ...
  function renderSetScores(state) {
    const root = document.getElementById('setScores');
    ...
    const addValues = (team, values) => {
      const rects = layout.setScores?.[team] ?? [];
      values.forEach((value, index) => {
        const cfg = rects[index];
        if (!cfg) return;

        const el = document.createElement('div');
        el.className = `set-score ${team === 'A' ? 'red' : 'blue'}`;
        el.textContent = String(value ?? 0);
        setRect(el, cfg);
        el.style.fontSize = `${cfg.fontSize}px`;
        root.appendChild(el);
