(() => {
  ...
  function renderSetScores(state) {
    const valuesA = Array.isArray(state.setsA) ? state.setsA.slice(0, 3) : [];
    const valuesB = Array.isArray(state.setsB) ? state.setsB.slice(0, 3) : [];

    for (const [team, values] of [['A', valuesA], ['B', valuesB]]) {
      const positions = layout.setScores?.[team] ?? [];
      for (let i = 0; i < 3; i++) {
        const cfg = positions[i];
        if (!cfg) continue;

        const el = document.createElement('div');
        el.className = `set-score-number ${team === 'A' ? 'red' : 'blue'}`;
  ...
  function renderCounters(state) {
    // El PSD corregido ya no muestra un contador verde de sets ganados.
    // En su lugar cada columna de SETS muestra el resultado numérico real.
    const values = {
